...
struct AI_data{
	...
	// about its old position, as well as current velocity vector.
	// NOTE: The estimates of velocity kept here can be different
	//       from the velocity estimates for the blobs corresponding
	//       to each agent. In particular for the ball. See which 
        //       estimate you find more reliable and use that one.
	//
	// MIND THE NOISE.
	...
	int side;		// side=0 implies the robot's own side is the left side
				...
				// on the field
        int botCol;		// Own bot's colour. 0 - green, 1 - blue
...
void init_blob(struct blob *myblob, struct blob *b, double height);
